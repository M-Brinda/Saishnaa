...
const port = 9090;
const orgId = 'team_zJMtw81kcz2U2rqPKGERNfQ8';
const projectName = 'saishnaa-uu7d';
...
const server = http.createServer((req, res) => {
  ...
  if (urlPath === '/' || urlPath === '/deploy-dash.html') {
    ...
    if (!token) {
      ...
      res.end(JSON.stringify({ error: { message: 'Missing VERCEL_TOKEN environment variable' } }));
      return;
    }
    ...
    try {
      ...
      const deployReq = require('https').request({
        ...
        deployRes.on('end', () => {
          res.writeHead(deployRes.statusCode, { 'Content-Type': 'application/json' });
